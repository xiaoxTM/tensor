...
#include <libu/u-log>
 ...
 void tensor_test() {
     ...
     u::tensor::Tensor tc = u::tensor::concatenate({t8, t9}, 2);
     std::cout << tc << std::endl;

     std::vector<u::tensor::Tensor> splits = u::tensor::split(t9, {1,2,3,2}, -1);
     for (size_t s=0; s<splits.size(); ++s) {
         std::cout << splits[s] << std::endl;
     }
...
void random_test() {
    u::tensor::Tensor t = u::tensor::random::bernoulli(u::tensor::Shape({2,3,5}), u::tensor::DType::int32, 0.4);
    std::cout << t << std::endl;

    u::tensor::Tensor t0 = u::tensor::random::binomial<int>(u::tensor::Shape({4,2,5}), u::tensor::DType::int32, 4, 0.5);
    std::cout << t0 << std::endl;

    u::tensor::Tensor t1 = u::tensor::random::cauchy<float>(u::tensor::Shape({4,2,3}), u::tensor::DType::float32, 0.1, 0.5);
